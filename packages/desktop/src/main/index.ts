...
const APP_IDS: Record<string, string> = {
  dev: "ai.kursor.desktop.dev",
  beta: "ai.kursor.desktop.beta",
  prod: "ai.kursor.desktop",
}
...
const main = Effect.gen(function* () {
  ...
  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.kursor.desktop.dev"
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "kursor Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("kursor://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("kursor")
  registerRendererProtocol()
  setDockIcon()
  setupAutoUpdater()
  ...
  const needsMigration = ((): boolean => {
    ...
    return !existsSync(join(base, "kursor", "kursor.db"))
  })()

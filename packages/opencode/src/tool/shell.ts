...
import { detectDestructive, formatDestructive } from "./shell/destructive"
...
const ask = Effect.fn("ShellTool.ask")(function* (ctx: Tool.Context, scan: Scan, command: string) {
  // Surface a destructive-pattern warning on every shell permission request so
  // the renderer can display it next to Allow / Deny. Purely informational —
  // permission grant/deny logic is unaffected.
  const destructive = detectDestructive(command)
  const destructiveMeta = destructive
    ? {
        destructive_warnings: destructive.map((w) => ({ message: w.warning, severity: w.severity })),
        destructive_summary: formatDestructive(destructive),
        destructive_severity: destructive[0]?.severity ?? "medium",
      }
    : {}

...
const DEBUG_BAR_KEY = "kursor.debugBar.v1"
type DebugBarPersist = {
  visible: boolean
  pos?: { x: number; y: number }
}

function loadDebugBarState(): DebugBarPersist {
  if (typeof localStorage === "undefined") return { visible: false }
  try {
    const raw = localStorage.getItem(DEBUG_BAR_KEY)
    if (!raw) return { visible: false }
    const parsed = JSON.parse(raw) as DebugBarPersist
    return { visible: !!parsed.visible, pos: parsed.pos }
  } catch {
    return { visible: false }
  }
}

function saveDebugBarState(state: DebugBarPersist) {
  if (typeof localStorage === "undefined") return
  try {
    localStorage.setItem(DEBUG_BAR_KEY, JSON.stringify(state))
  } catch {
    // ignore
  }
}

const [debugBarVisible, setDebugBarVisibleSignal] = createSignal(loadDebugBarState().visible)

export function isDebugBarVisible() {
  return debugBarVisible()
}

export function toggleDebugBar() {
  setDebugBarVisibleSignal((prev) => {
    const next = !prev
    const current = loadDebugBarState()
    saveDebugBarState({ ...current, visible: next })
    return next
  })
}

export function setDebugBarVisible(value: boolean) {
  setDebugBarVisibleSignal(() => {
    const current = loadDebugBarState()
    saveDebugBarState({ ...current, visible: value })
    return value
  })
}
...
export function DebugBar() {
  ...
  const persisted = loadDebugBarState()
  const [pos, setPos] = createSignal<{ x: number; y: number }>(
    persisted.pos ?? (() => {
      if (typeof window === "undefined") return { x: 16, y: 16 }
      const w = window.innerWidth
      const h = window.innerHeight
      return { x: Math.max(0, w - 340), y: Math.max(0, h - 110) }
    })(),
  )
  const [dragging, setDragging] = createSignal(false)

  function startDrag(e: PointerEvent) {
    if ((e.target as HTMLElement | null)?.closest("[data-debug-bar-close]")) return
    setDragging(true)
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      const barH = 100
      const next = {
        x: Math.min(Math.max(0, origin.x + dx), Math.max(0, w - bar)),
        y: Math.min(Math.max(0, origin.y + dy), Math.max(0, h - barH)),
      }
      setPos(next)
    }
    const onUp = () => {
      setDragging(false)
      const current = loadDebugBarState()
      saveDebugBarState({ ...current, pos: pos() })
  ...
  return (
    <Show when={debugBarVisible()}>
    <aside
      aria-label={language.t("debugBar.ariaLabel")}
      onPointerDown={startDrag}
      style={{ left: `${pos().x}px`, top: `${pos().y}px` }}
      classList={{
        "pointer-events-auto fixed z-50 w-[308px] max-w-[calc(100vw-1.5rem)] overflow-hidden rounded-xl border border-border-base bg-surface-raised-stronger-non-alpha p-0.5 text-text-strong shadow-[var(--shadow-lg-border-base)] sm:w-[324px] select-none":
          true,
    ...
    >
      <button
        data-debug-bar-close
        onClick={(e) => {
          e.stopPropagation()
          setDebugBarVisible(false)
        }}
        aria-label="Hide debug bar"
        class="absolute top-1 right-1 z-10 size-5 rounded-md text-text-weak hover:text-text-strong hover:bg-surface-pressed flex items-center justify-center text-xs leading-none"
      >
        ×
      </button>
